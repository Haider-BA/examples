// Test DMDAGetGlobalIndices

char help[] = "Test DA and DMDAGetGlobalIndices\n";
...
int main(int argc,char **argv)
{
	...
	ierr = PetscPrintf(PETSC_COMM_WORLD, "\nSet all elements of the global vector with the same value [VecSet]"); CHKERRQ(ierr);
	hline();
	...
	ierr = PetscPrintf(PETSC_COMM_WORLD, "\nSet elements of the global vector with different values [VecSetValues, VecAssemblyBegin and VecAssemblyEnd]"); CHKERRQ(ierr);
	hline();
	ierr = VecView(global,PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);
...
/*
	// transfer from local arrays to global array
